...
export const authenticateWidget = async (req, res, next) => {
  try {
    ...
    if (origin && project.allowed_domains && Array.isArray(project.allowed_domains) && project.allowed_domains.length > 0) {
...
export const validateWidgetConfig = (req, res, next) => {
  try {
    ...
    if (description.length > 10000) {
      throw createAppError('Bug description must be less than 10,000 characters', 400);
    }
...
export const sanitizeWidgetData = (req, res, next) => {
  try {
    const { title, description, reporter_email, error_details, environment } = req.body;
    ...
    if (error_details && typeof error_details === 'object') {
      ...
      const allowedErrorFields = ['message', 'stack', 'filename', 'lineno', 'colno', 'type'];
      allowedErrorFields.forEach(field => {
        if (error_details[field] !== undefined) {
          sanitizedErrorDetails[field] = String(error_details[field]).substring(0, 5000); // Limit length
        }
      });
    ...
    if (environment && typeof environment === 'object') {
      ...
      const allowedEnvFields = ['userAgent', 'url', 'timestamp', 'viewport', 'language'];
      allowedEnvFields.forEach(field => {
        if (environment[field] !== undefined) {
          sanitizedEnvironment[field] = String(environment[field]).substring(0, 1000); // Limit length
        }
      });
