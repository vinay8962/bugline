...
export const createUser = async (userData) => {
  ...
  try {
    ...
    const emailVerified =
      globalRole === "SUPER_ADMIN" || globalRole === "ADMIN" ? true : false;
    ...
    logPerformance("User creation", duration, {
      userId: user.id,
      email: user.email,
    });
    logUserAction(user.id, "user_created", {
      email: user.email,
      role: globalRole,
    });
...
export const getUserByEmail = async (email) => {
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    logError("Get user by email failed", {
      error: error.message,
      email,
      duration,
    });
...
export const getUserById = async (userId) => {
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    logError("Get user by ID failed", {
      error: error.message,
      userId,
      duration,
    });
...
export const updateUser = async (userId, updateData) => {
  ...
  try {
    ...
    logUserAction(userId, "profile_updated", {
      updatedFields: Object.keys(transformedData),
    });
...
export const getAllUsers = async (page = 1, limit = 10, filters = {}) => {
  try {
    ...
    // Transform company data for each user
    const usersWithTransformedCompanies = users.map((user) => {
      const { company_users, ...userWithoutCompanyUsers } = user;
    ...
    return {
      users: usersWithTransformedCompanies,
      pagination: {
        page,
        limit,
        total,
