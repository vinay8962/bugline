  ...
  endpoints: (builder) => ({
    ...
    getProjectsByCompany: builder.query({
      query: ({ companyId, page = 1, limit = 20, status = '' } = {}) => ({
        url: `${API_ENDPOINTS.PROJECTS.LIST_BY_COMPANY(companyId)}?page=${page}&limit=${limit}&status=${status}`,
      }),
    ...
    searchProjectsByCompany: builder.query({
      query: ({ companyId, query = '', status = '', page = 1, limit = 20 } = {}) => ({
        url: `${API_ENDPOINTS.PROJECTS.SEARCH_BY_COMPANY(companyId)}?q=${query}&status=${status}&page=${page}&limit=${limit}`,
      }),
      providesTags: ['Project'],
    }),
