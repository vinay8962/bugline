...
import { Building2, FolderPlus, Users, Calendar, BarChart3, Plus, ChevronDown } from "lucide-react";
import { useGetCompanyDetailsQuery, useGetCompanyStatsQuery } from "../services/companyApi.js";
import { useGetProjectsByCompanyQuery } from "../services/projectApi.js";
import { useGetCurrentUserQuery, useGetUserByIdQuery } from "../services/userApi.js";
...
const CompanyDashboard = ({ companyId, companyRole }) => {
  ...
  } = useGetProjectsByCompanyQuery({ companyId: selectedCompanyId }, {
    skip: !selectedCompanyId,
  });

  const companies = userWithCompaniesData?.data?.companies || [];
  const adminCompanies = companies.filter(company => company?.role === COMPANY_ROLES.ADMIN);
  ...
  useEffect(() => {
    if (!selectedCompanyId && adminCompanies.length > 0 && adminCompanies[0]?.id) {
      setSelectedCompanyId(adminCompanies[0].id);
    }
  }, [selectedCompanyId, adminCompanies]);

  // Handle loading state
  if (loadingCurrentUser || loadingUserCompanies || loadingCompany || loadingProjects) {
    return (
  ...
  if (!loadingUserCompanies && adminCompanies.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-semibold text-white mb-2">No Admin Companies</h3>
        <p className="text-gray-400">You are not an admin of any companies yet.</p>
      </div>
    );
  }
      ...
      {adminCompanies.length > 1 && (
        <div className="bg-primary border border-gray-800 rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-white">Company Dashboard</h3>
              <p className="text-gray-400 text-sm">Admin of {adminCompanies.length} companies</p>
            </div>
            <div className="relative">
              ...
              >
                ...
                <span>{adminCompanies.find(c => c.id === selectedCompanyId)?.name || 'Select Company'}</span>
            ...
            <div>
              ...
              <div className="flex items-center gap-4 mt-2 text-sm text-gray-400">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span>Since {company ? new Date(company.created_at).toLocaleDateString() : ''}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {!loadingStats && stats && Object.keys(stats).length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-primary border border-gray-800 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-white mt-1">{stats.totalBugs || 0}</p>
          </div>
          <div className="bg-primary border border-gray-800 rounded-lg p-4">
            <div className="flex items-center gap-2">
            ...
            <p className="text-2xl font-bold text-white mt-1">{stats.openBugs || 0}</p>
          </div>
          <div className="bg-primary border border-gray-800 rounded-lg p-4">
            <div className="flex items-center gap-2">
              <BarChart3 className="w-5 h-5 text-blue-500" />
            ...
            <p className="text-2xl font-bold text-white mt-1">{stats.inProgressBugs || 0}</p>
          </div>
          <div className="bg-primary border border-gray-800 rounded-lg p-4">
            <div className="flex items-center gap-2">
            ...
            <p className="text-2xl font-bold text-white mt-1">{stats.resolvedBugs || 0}</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-primary border border-gray-800 rounded-lg p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-400 text-sm">Manage your company projects</p>
          </div>
        ...
        {projects.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-400 mb-6 max-w-md mx-auto">
              Get started by creating your first project to organize and track bugs effectively.
            </p>
            {isAdmin && (
              <button
                onClick={() => setShowCreateProject(true)}
                ...
                {project.bug_stats && (
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-blue-300">{project.bug_stats.in_progress}</span>
                    </div>
                    <div className="flex justify-between">
